...
class ControllerSellerSeller extends Controller {
	...
	public function approve() {
		...
		if (isset($this->request->get['seller_id']) && $this->validateApprove()) {
	...
	protected function getList() {
		...
		$data['approve'] = $this->url->link('seller/seller/approve', 'token=' . $this->session->data['token'] . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->error['company_nature'])) {
			...
			$data['error_company_nature'] = '';
		}
		
		if (isset($this->error['business_nature'])) {
			$data['error_business_nature'] = $this->error['business_nature'];
		} else {
			$data['error_business_nature'] = '';
		}
		
		if (isset($this->error['cancelled_cheque'])) {
			$data['error_cancelled_cheque'] = $this->error['cancelled_cheque'];
		} else {
			$data['error_cancelled_cheque'] = '';
		}
		...
		if (isset($this->error['tin_number'])) {
			$data['error_tin_number'] = $this->error['tin_number'];
		} else {
			$data['error_tin_number'] = '';
		}
		
		if (isset($this->error['aadhar_number'])) {
			$data['error_aadhar_number'] = $this->error['aadhar_number'];
		} else {
			$data['error_aadhar_number'] = '';
		}
    ...
    // Logo
		...
		if (isset($this->request->post['company_nature'])) {
			$data['company_nature'] = $this->request->post['company_nature'];
		} elseif (!empty($seller_info)) {
			$data['company_nature'] = $seller_info['company_nature'];
		} else {
			$data['company_nature'] = '';
		}
		if (isset($this->request->post['business_nature'])) {
			$data['business_nature'] = $this->request->post['business_nature'];
		} elseif (!empty($seller_info)) {
			$data['business_nature'] = $seller_info['business_nature'];
		} else {
			$data['business_nature'] = '';
		}
		if (isset($this->request->post['cancelled_cheque'])) {
			$data['cancelled_cheque'] = $this->request->post['cancelled_cheque'];
		} elseif (!empty($seller_info)) {
			$data['cancelled_cheque'] = $seller_info['cancelled_cheque'];
		} else {
			$data['cancelled_cheque'] = '';
		}
		if (isset($this->request->post['tin_number'])) {
			$data['tin_number'] = $this->request->post['tin_number'];
		} elseif (!empty($seller_info)) {
			$data['tin_number'] = $seller_info['tin_number'];
		} else {
			$data['tin_number'] = '';
		}
		if (isset($this->request->post['aadhar_number'])) {
			$data['aadhar_number'] = $this->request->post['aadhar_number'];
		} elseif (!empty($seller_info)) {
			$data['aadhar_number'] = $seller_info['aadhar_number'];
		} else {
			$data['aadhar_number'] = '';
		}
		
		if (isset($this->request->post['bank_statement']) && is_file(DIR_IMAGE . $this->request->post['bank_statement'])) {
			$data['bank_statement'] = $this->model_tool_image->resize($this->request->post['bank_statement'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['bank_statement'])) {
			$data['bank_statement'] = $this->model_tool_image->resize($seller_info['bank_statement'], 100, 100);
		} else {
			$data['bank_statement'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if (isset($this->request->post['aadhar_upload']) && is_file(DIR_IMAGE . $this->request->post['aadhar_upload'])) {
			$data['aadhar_upload'] = $this->model_tool_image->resize($this->request->post['aadhar_upload'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['aadhar_upload'])) {
			$data['aadhar_upload'] = $this->model_tool_image->resize($seller_info['aadhar_upload'], 100, 100);
		} else {
			$data['aadhar_upload'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if (isset($this->request->post['proof_upload']) && is_file(DIR_IMAGE . $this->request->post['proof_upload'])) {
			$data['proof_upload'] = $this->model_tool_image->resize($this->request->post['proof_upload'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['proof_upload'])) {
			$data['proof_upload'] = $this->model_tool_image->resize($seller_info['proof_upload'], 100, 100);
		} else {
			$data['proof_upload'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if (isset($this->request->post['certificate_upload']) && is_file(DIR_IMAGE . $this->request->post['certificate_upload'])) {
			$data['certificate_upload'] = $this->model_tool_image->resize($this->request->post['certificate_upload'], 100, 100);
		} elseif (!empty($seller_info) && is_file(DIR_IMAGE . $seller_info['certificate_upload'])) {
			$data['certificate_upload'] = $this->model_tool_image->resize($seller_info['certificate_upload'], 100, 100);
		} else {
			$data['certificate_upload'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		$this->load->model('tool/upload');
		
			$upload_info = $this->model_tool_upload->getUploadByCode($seller_info['bank_statement']);  
			//print_r($upload_info);exit;
			if(isset($upload_info['name'])){
				...
				//echo "<pre>";print_r(is_file(DIR_IMAGE.$upload_info['filename']));exit;
				$data['bank_download_url'] = '../system/storage/upload/'.$upload_info['name'];
			}else{
				$data['bank_download_url'] = "";
			}
		
			$upload_info = $this->model_tool_upload->getUploadByCode($seller_info['aadhar_upload']);  
			//print_r($upload_info);exit;
			if(isset($upload_info['name'])){
				//print_r($upload_info);exit;
				$phyname = DIR_UPLOAD.$upload_info['filename'];
				...
				copy($phyname,$temp_name);
				//echo "<pre>";print_r(is_file(DIR_IMAGE.$upload_info['filename']));exit;
				$data['aadhar_download_url'] = '../system/storage/upload/'.$upload_info['name'];
			}else{
				$data['aadhar_download_url'] = "";
			}

			$upload_info = $this->model_tool_upload->getUploadByCode($seller_info['proof_upload']);  
			if(isset($upload_info['name'])){
				//print_r($upload_info);exit;
				$phyname = DIR_UPLOAD.$upload_info['filename'];
				$temp_name = DIR_UPLOAD.$upload_info['name'];
				copy($phyname,$temp_name);
				//echo "<pre>";print_r(is_file(DIR_IMAGE.$upload_info['filename']));exit;
				$data['proof_download_url'] = '../system/storage/upload/'.$upload_info['name'];
			}else{
				$data['proof_download_url'] = "";
			} 

			$upload_info = $this->model_tool_upload->getUploadByCode($seller_info['certificate_upload']);  
			if(isset($upload_info['name'])){
				//print_r($upload_info);exit;
				$phyname = DIR_UPLOAD.$upload_info['filename'];
				$temp_name = DIR_UPLOAD.$upload_info['name'];
				copy($phyname,$temp_name);
				//echo "<pre>";print_r(is_file(DIR_IMAGE.$upload_info['filename']));exit;
				$data['company_download_url'] = '../system/storage/upload/'.$upload_info['name'];
			}else{
				$data['company_download_url'] = "";
			} 
		...
		if (isset($this->request->post['commission'])) {
			...
			$data['commission'] = $seller_info['commission'];
		} else {
			$data['commission'] = $this->config->get('config_seller_commission');
		}
		...
		if (isset($this->request->post['is_paid'])) {
			$data['is_paid'] = $this->request->post['is_paid'];
		} elseif (!empty($seller_info)) {
			$data['is_paid'] = $seller_info['is_paid'];
		} else {
			$data['is_paid'] = true;
		}
           
                $this->load->model('seller/seller_group');
		$data['group_lists'] = $this->model_seller_seller_group->getAllgroupsReg();
	...
	public function autocomplete() {
		...
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($seller_data));
	}
}
