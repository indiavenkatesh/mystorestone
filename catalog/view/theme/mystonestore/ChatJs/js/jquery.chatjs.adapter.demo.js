...
var DemoServerAdapter = (function () {
    ...
    DemoServerAdapter.prototype.sendMessage = function (roomId, conversationId, otherUserId, messageText, clientGuid, done) {
        ...
        bounceMessage.UserToId = DemoAdapterConstants.otherUserId; // ... to the Echobot
        ...
        setTimeout(function () {
            _this.getUserInfo(otherUserId, function (echobotUserInfo) {
                ...
                typingSignal.UserFrom = CURRENT_USER_ID;

                // if it's not a private message, the echo message will be to the current user
                if (!roomId && !conversationId)
                    typingSignal.UserToId = DemoAdapterConstants.otherUserId;
                ...
                setTimeout(function () {
                    ...
                    echoMessage.UserFromId = DemoAdapterConstants.CURRENT_USER_ID; // It will be from Echobot
                    ...
                    // if it's not a private message, the echo message will be to the current user
                    if (!roomId && !conversationId)
                        echoMessage.UserToId = DemoAdapterConstants.otherUserId;
    ...
    DemoServerAdapter.prototype.getUserList = function (roomId, conversationId, done) {
        ...
        $.ajax({
            url : "index.php?route=simple_blog/article/getuserlists",
            type : "POST",
            dataType: 'json',
            context:this,
            success : function(data) {
                ...
                $.each(data, function(index) {
                    ...
                    tmpUser.Id                  = data[index].customer_id;
                    tmpUser.RoomId              = data[index].customer_group_id;
                    tmpUser.Name                = data[index].firstname+' '+data[index].lastname;
                    tmpUser.Email               = data[index].email;
                    tmpUser.ProfilePictureUrl   = "http://placehold.it/50/55C1E7/fff&text=" + data[index].firstname.slice(0,2);
                    tmpUser.Status              = "1";
