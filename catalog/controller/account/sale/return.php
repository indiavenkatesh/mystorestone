<?php
class ControllerAccountSaleReturn extends Controller {
	private $error = array();
    ...
    if (!$this->customer->isLogged()) {
		...
		$this->load->language('account/sale/return');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('account/sale/return');
    ...
    if (!$this->customer->isLogged()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this-> model_account_sale_return->addReturn($this->request->post);
			...
			$this->response->redirect($this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . $url, true));
		}
    ...
    if (!$this->customer->isLogged()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this-> model_account_sale_return->editReturn($this->request->get['return_id'], $this->request->post);
    ...
    if (!$this->customer->isLogged()) {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $return_id) {
				$this-> model_account_sale_return->deleteReturn($return_id);
			}
    ...
    if (!$this->customer->isLogged()) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('account/sale/return/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('account/sale/return/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$return_total = $this-> model_account_sale_return->getTotalReturns($filter_data);

		$results = $this-> model_account_sale_return->getReturns($filter_data);
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'edit'          => $this->url->link('account/sale/return/edit', 'token=' . $this->session->data['token'] . '&return_id=' . $result['return_id'] . $url, true)
			);
		}
		...
		$data['sort_return_id'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=r.return_id' . $url, true);
		$data['sort_order_id'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=r.order_id' . $url, true);
		$data['sort_customer'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, true);
		$data['sort_product'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=product' . $url, true);
		$data['sort_model'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=model' . $url, true);
		$data['sort_status'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		$data['sort_date_added'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=r.date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . '&sort=r.date_modified' . $url, true);
		...
		$pagination->url = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('account/sale/return_list', $data));
	}
    ...
    if (!$this->customer->isLogged()) {
		...
		if (!isset($this->request->get['return_id'])) {
			$data['action'] = $this->url->link('account/sale/return/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('account/sale/return/edit', 'token=' . $this->session->data['token'] . '&return_id=' . $this->request->get['return_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('account/sale/return', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['return_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$return_info = $this-> model_account_sale_return->getReturn($this->request->get['return_id']);
		}
		...
		$this->response->setOutput($this->load->view('account/sale/return_form', $data));
	}
    ...
    if ($this->customer->hasSellerPermission() == 0) {
			...
			if (!$data['error']) {
				$this-> model_account_sale_return->addReturnHistory($this->request->get['return_id'], $this->request->post);
		...
		$results = $this-> model_account_sale_return->getReturnHistories($this->request->get['return_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this-> model_account_sale_return->getTotalReturnHistories($this->request->get['return_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('account/sale/return/history', 'token=' . $this->session->data['token'] . '&return_id=' . $this->request->get['return_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('account/sale/return_history', $data));
	}
}