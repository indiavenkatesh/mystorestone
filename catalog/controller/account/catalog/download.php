<?php
class ControllerAccountCatalogDownload extends Controller {
	private $error = array();
  ...
  $seller_id = $this->customer->getSellerId();
    if (!$this->customer->isLogged()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_account_catalog_download->addDownload($this->request->post);
			...
			$this->response->redirect($this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . $url, true));
		}
  ...
  $seller_id = $this->customer->getSellerId();
    if (!$this->customer->isLogged()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_account_catalog_download->editDownload($this->request->get['download_id'], $this->request->post);
    ...
    if (!$this->customer->isLogged()) {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $download_id) {
				$this->model_account_catalog_download->deleteDownload($download_id);
			}
  ...
  $seller_id = $this->customer->getSellerId();
    if (!$this->customer->isLogged()) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('account/catalog/download/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('account/catalog/download/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$download_total = $this->model_account_catalog_download->getTotalDownloads();

		$results = $this->model_account_catalog_download->getDownloads($filter_data);
		...
		foreach ($results as $result) {
			$data['downloads'][] = array(
				...
				'edit'        => $this->url->link('account/catalog/download/edit', 'token=' . $this->session->data['token'] . '&download_id=' . $result['download_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . '&sort=dd.name' . $url, true);
		$data['sort_date_added'] = $this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . '&sort=d.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('account/catalog/download_list', $data));
	}
  ...
  $seller_id = $this->customer->getSellerId();
    if (!$this->customer->isLogged()) {
		...
		if (!isset($this->request->get['download_id'])) {
			$data['action'] = $this->url->link('account/catalog/download/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('account/catalog/download/edit', 'token=' . $this->session->data['token'] . '&download_id=' . $this->request->get['download_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('account/catalog/download', 'token=' . $this->session->data['token'] . $url, true);
		...
		if (isset($this->request->get['download_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$download_info = $this->model_account_catalog_download->getDownload($this->request->get['download_id']);
		}
		...
		if (isset($this->request->post['download_description'])) {
			...
			$data['download_description'] = $this->model_account_catalog_download->getDownloadDescriptions($this->request->get['download_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('account/catalog/download_form', $data));
	}
  ...
  $seller_id = $this->customer->getSellerId();
    ...
    $customer_id = $this->customer->getId();
		if (!is_file(DIR_DOWNLOAD . '/' . $customer_id . '/' . $this->request->post['filename'])) {
  ...
  $seller_id = $this->customer->getSellerId();
		...
		foreach ($this->request->post['selected'] as $download_id) {
			$product_total = $this->model_account_catalog_product->getTotalProductsByDownloadId($download_id);
