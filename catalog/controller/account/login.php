...
class ControllerAccountLogin extends Controller {
	...
	public function index() {
		...
		if (!empty($this->request->get['token'])) {
			...
			$this->model_account_customer->setCustomerOnlineStatus($this->customer->getId(),0);
    ...
    if ($this->customer->isLogged() && isset($this->request->get['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
      ...
      $this->session->data['token'] = token(32);
			...
			//update online status
			$this->model_account_customer->setCustomerOnlineStatus($this->customer->getId(),1);
			// Add to activity log
	...
	protected function validate() {
		...
		if(is_numeric($emails)){
			$customer_phone_info = $this->model_account_customer->getCustomerByPhone($this->request->post['email']);
			if($customer_phone_info){
				$this->request->post['email'] = $customer_phone_info['email'];
			} else {
