...
class ControllerCustomerCustomer extends Controller {
  ...
  public function sellerapprove() {
	...
	protected function getList() {
      ...
      if (!$result['sellerapprove']) {
				$sellerapprove = $this->url->link('customer/customer/sellerapprove', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, true);
			} else {
				$sellerapprove = '';
			}
    ...
    $data['button_seller_approve'] = $this->language->get('button_seller_approve');
    ...
    if (isset($this->request->post['seller_approved'])) {
			...
			$data['seller_approved'] = $seller_info['approved'];
		} else {
